...
public class Demo2 {
    public static void main(String[] args) {
        try {
            File myObj = new File("D:\\TheNewMoonST\\file\\domain_file\\canvasLink.txt");
            ...
            while (myReader.hasNextLine()) {
                ...
                while (check)
                {
                    ...
                    if (isJSONValid(outputResult)){
                        ...
                        if (obj.get("images").toString().isEmpty() == true){
                            check = false ;
                        }else {
...
//                                SaveFile(data+"/products/"+obj.get("handle").toString());
    ...
    public static void SaveFile(String data) throws IOException {
        BufferedWriter writer = new BufferedWriter(new FileWriter("D:\\TheNewMoonST\\file\\domain_file\\canvasHandle.txt", true));
        writer.write(data + "\n");
        writer.close();
    }
}
